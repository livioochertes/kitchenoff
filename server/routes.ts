...
export async function registerRoutes(app: Express): Promise<Server> {
  // Admin interface route (production - no Babel)
  app.get("/admin", (req, res) => {
    try {
      const adminPath = path.resolve('./admin/admin-production.html');
      console.log('Serving production admin interface from:', adminPath);
      ...
      console.error('Error serving production admin interface:', error);
      res.status(500).send('Admin interface not available');
    }
  });

  // Full admin interface route (complete features with Babel)
